...
template <typename T> Vec<tuple<i32, i32, T>> runlength(const Vec<T> &a) {
  if (a.empty()) {
...
template <unsigned mod> constexpr unsigned primitive_root() {
...
// y >= 1
template <typename T> constexpr T safe_mod(T x, T y) {
  x %= y;
  if (x < 0) {
    x += y;
  }
...
// y != 0
template <typename T> constexpr T floor_div(T x, T y) {
  if (y < 0) {
    x *= -1;
    y *= -1;
  }
...
// y != 0
template <typename T> constexpr T ceil_div(T x, T y) {
  if (y < 0) {
    x *= -1;
    y *= -1;
  }
...
template <typename T> vector<T> SuffixSum(vector<T> X) {
  int n = X.size();
  vector<T> ret(n, 0);
  for (int i = n - 1; i >= 0; i--) {
    ret[i] = X[i];
...
template <typename T> void print(Vec<Vec<T>> v) {
  i32 N = v.size();
  REP(i, N) {
    for (auto &x : v[i])
      cout << x << " ";
...
using pl = pair<ll, ll>;
const ll N = 4e5 + 20;
FactorialTable<Mint> tab(N);
// ============
Mint get_ans(Vec<ll> &a) {
  ll n = a.size();
  map<ll, ll> mp;
  for (ll i = 0; i < n; i++)
    mp[a[i]]++;
...
void solve() {
  ...
  sort(begin(a), end(a), [&](auto x, auto y) { return x.first < y.first; });
  Vec<ll> b, c, d;
  for (ll i = 0; i < n; i++) {
    b.pb(a[i].first);
  }
  sort(begin(a), end(a), [&](auto x, auto y) { return x.second < y.second; });
  for (ll i = 0; i < n; i++) {
    c.pb(a[i].second);
  }
  bool ok = true;
