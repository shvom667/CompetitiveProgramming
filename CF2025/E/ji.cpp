...
vm distribute(const lli M,const lli mn){
    const lli L = mn+M+2;
    vm dp(L);
    dp[mn]=1;
    for(lli k=M;k>=1;k--){
        vm ndp(L);
        for(lli pd=-mn;pd<=M;pd++){
            const lli idx = pd+mn;
            if(idx+1<L)
                ndp[idx+1]+=dp[idx];
            if(idx>0)
...
while(T--)
{
    lli n,m;
    cin>>n>>m;
    auto dp = distribute(m,0);
    vm rest(m+1);
    for(lli df=0;df<=m;df++){
        const auto pr=distribute(m,df);
        rest[df]=pr[0];
    }
    dbg(dp);
    dbg(rest);
    for(lli jt=1;jt<n;jt++){
        vm ndp(m+1);
        for(lli pk=0;pk<=m;pk++)
        for(lli k=0;k<=pk;k++)
