...
auto solve(string s) {
    ...
    for (ll ci = 0; ci <= count[0]; ci++) {
        dp[0][ci][ci][0] = table.fact(ci) * table.fact(count[0] - ci);
    }

    for (ll i = 1; i < 10; i++) {
        for (ll ci = 0; ci <= count[i]; ci++) {
            for (ll c = ci; c <= 40; c++) {
                for (ll s = 0; s <= 400; s++) {
                    if (ci == 0) {
                        for (ll k = 0; k <= count[i - 1]; k++) {
                            dp[i][ci][c][s] += dp[i- 1][k][c][s];
                        }
                        dp[i][ci][c][s] /= table.fact(ci);
                        dp[i][ci][c][s] /= table.fact(count[i] - ci);
                    }
                    if (ci > 0 && s >= i) {
                        for (ll k = 0; k <= count[i - 1]; k++) {
                            if (s - ci * i >= 0 && c >= ci)
                            dp[i][ci][c][s] += dp[i- 1][k][c - ci][s - ci * i];
                        }
                        dp[i][ci][c][s] /= table.fact(ci);
                        dp[i][ci][c][s] /= table.fact(count[i] - ci);
                    }
    ...
    for (ll c = 0; c <  10; c++) {
        fans += dp[9][c][n / 2][sum / 2];
    }

    fans *= table.fact(n / 2);
    fans *= table.fact((n + 1) / 2);

    int res = (int)fans.get_val();
    return res;
}
