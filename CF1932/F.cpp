...
void solve() {
    ...
    Vec<ll> cvg(n+2, 0);
    Vec<pl> sg(m);
    Vec<Vec<ll>> st(n+2), en(n+2),st2(n+2),en2(n+2);
    Vec<ll> fst(n+2), lst(n+2);
    REP (i, m) {
        cin >> sg[i].first >> sg[i].second;
        cvg[sg[i].first]+=1;
        cvg[sg[i].second+1]+=-1;
        en[sg[i].second].pb(sg[i].first);
        st[sg[i].first].pb(sg[i].second);
    }
    for (ll i=0;i<=n+1;i++) {
        if(i) {
            cvg[i] += cvg[i-1];
    ...
    dbg(en);
    dbg(st);
    for (ll i=0;i<=n;i++) {fst[i]=i+1;lst[i]=i-1;}
    multiset<ll> a,b;
    for (ll i=1;i<=n;i++){
        for (auto&ep:st[i]) {
            b.insert(ep);
    ...
    for (ll i=0;i<=n;i++) {
        if(fst[i]>lst[i]) fst[i]=lst[i]=-1;
    }
    REP(i,1,n+1) {dbg(i,fst[i],lst[i]);}
    Vec<ll> dp(n+1, 0);
    Lsegtree<ll,ll> seg(n+2,-INF64,-INF64);
    Vec<ll> dummy(n+2,0);
    seg.build(dummy);
    for (ll i=1;i<=n;i++) {
        if(fst[i]==-1) continue;
        dp[i] = cvg[i]+seg.query(0,fst[i]-1);
        ll v=seg.query(i,i);
        chmax(v,dp[i]);
        seg.update(i,i,v);
    }
