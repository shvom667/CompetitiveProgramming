...
auto solve(ll n, vector<string> s) {
  Mint fans = 0;
  ll w, b; w = b = 0;
  ...
  bool ok = true;
  for (ll i = 0; i < n; i++) {
    if (s[i] == (string)"WW" || s[i] == (string)"BB") {
      ok = false;
    }
  }    
  if (ok) {
    ...
    ok1 = ok2 = false;
    for (ll i = 0; i < n; i++) {
      if (s[i] == (string)"WB" || s[i] == (string)"?B" || s[i] == (string)"??" || s[i] == (string)"W?") {
    ...
    for (ll i = 0; i < n; i++) {
      if (s[i] == (string)"BW" || s[i] == (string)"B?" || s[i] == (string)"??" || s[i] == (string)"?W") {
...
auto bf(ll n, vector<string> s) {
  ...
  for (ll mask = 0; mask < (1ll << sz); mask++) {
    ...
    for (ll bit = 0; bit < sz; bit++) {
      if (mask & (1ll << bit)) {
        r[q[bit].first][q[bit].second] = 'B';
      } else {
        r[q[bit].first][q[bit].second] = 'W';
      }
    }
    vector<ll> p;      
    bool ok = false;
    for (ll i = 0; i < n; i++) p.pb(i);
    do {
      Vec<string> rr = r;
      for (ll i = 0; i < n; i++) {
        rr[i] = r[p[i]];
      ...
      bool good = true;
      for (ll i = 0; i < n; i++) {
        if (rr[i][1] == rr[(i + 1) % n][0]) good = false;
      }
      if (good) {
        ok = true;
        break;
...
auto chk(ll n, vector<string> s) { 
  ...
  for (ll mask = 0; mask < (1ll << sz); mask++) {
    ...
    if (ok) {
      ll cwb = count(begin(r), end(r), (string)"WB");
      ll cbw = count(begin(r), end(r), (string)"BW");
      if (cbw + cwb == n && min(cbw, cwb) > 0) {
        ok = false;
      }
    }
    if (ok) {
...
// assuming the sufficient cond is cnt(w) == cnt(b) for a particular r -> not suffcient eg: WB BW
