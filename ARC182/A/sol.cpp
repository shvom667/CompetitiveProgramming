...
Mint to = 2;
auto solve(ll n, ll q, vector<ll> p, vector<ll> v)
{
	...
	for (ll i = 2; i <= q; i++) {
		for (ll x = 0; x < 2; x++) {
			for (ll j = 0; j < i - 1; j++) {
				if (v[i] >= v[i-1]) {
					if (x == 0 && (v[i] >= v[j] || p[i] < p[j])) {
						dp[i][j][x] += dp[i-1][j][x];
					}
					if (x == 1 && (v[i] >= v[j] || p[i] > p[j])) {
						dp[i][j][x] += dp[i-1][j][x];
					}
				}
			}
			for (ll k = 0; k < i - 1; k++) {
				...
				if (x == 0 && (v[i] >= v[i-1] || p[i] < p[i-1])) {
					
					if (i == 3 && x == 0) {
						dbg("hello", k);
					}
					dp[i][i-1][x] += dp[i-1][k][x^1];
				}
				if (x == 1 && (v[i] >= v[i-1] || p[i] > p[i-1])) {
					dp[i][i-1][x] += dp[i-1][k][x^1];
				}
			}
		}
