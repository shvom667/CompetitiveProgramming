...
auto solve() {
    ...
    while (q--) {
        ...
        if (type == 1) {
            ll val = na[x];
            na[x]++;
            auto it = upper_bound(begin(a),end(a),val);
            ll inda = it - begin(a) - 1;
            fans /= min(a[inda], b[inda]);
            a[inda]++;
            fans *= min(a[inda], b[inda]);
            cout<<fans<<'\n';
        } else {
            ll val = nb[x];
            nb[x]++;
            auto it = upper_bound(begin(b), end(b), val);
            ll indb = it - begin(b) - 1;
            fans /= min(a[indb], b[indb]);
            b[indb]++;
            fans *= min(a[indb], b[indb]);
            cout<<fans<<'\n';
        }
