...
FactorialTable<Mint> T(2e6 + 10);
Vec<ll> testCase(ll n) {
    if (n == 3) {
        return {2};
    }
    if (n == 4) {
    ...
    if (n == 10) {
        return {487, 162, 85, 10};
    }
    return {};
}
Mint g(ll x,ll y){
    if (y < x || x < 0) return Mint(0);
    return T.choose(x+y+1, y-x);
}
Vec<ll> solve(ll n) {
    Vec<Mint> fans;
    ...
    for (ll k = 1; k <= (n-1)/2; k++) {
        Mint cur = 0;
        for (ll x = 1; (2*k)*x+1 <= n; x++) {
            Mint val = T.choose(n,2*k*x) - g(k-1,n - 2*k*x+k-1);
            cur += val;
        }
        fans.pb(cur+1);
    }
