...
void solve() {
  ...
  for (ll i = n - 1; i >= 0; i--) {
    Mint k = i;
    Mint a = k; a/=n;
    Mint b = (((Mint)n-k)*k*dp[i+1]) / (((Mint)n)*n);
    Mint c = ((Mint)(n-k)*(k+1)*dp[i+1])/ ((Mint(n))*n);
    Mint d = (((Mint)(n-k))*(n-k-1)*dp[i+2])/(((Mint)n)*(n));
    Mint e = (Mint(n))*n-k*k;
    e /= (Mint)n*n;
    dp[i] = a+b+c+d;
    dp[i] /= e;
  }
  cout << dp[0] << " " << dp2[0] - dp[0] << "\n";
}
