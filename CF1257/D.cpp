...
void solve() {
  ...
  sort(begin(M), end(M), [&] (auto x, auto y) {
      if (x.first < y.first) {
        return true;
      }
      if (x.first > y.first) {
  ...
  ll sp = 1;
  ll day = 0;
  Lsegtree<ll, ll> seg(n+1, -INF64, -INF64);
  seg.build(a);

  while (sp <= n) {
    auto cond2 = [&] (ll x) {
      ll val = seg.query(sp, x);
      ll l = 0, r = (ll)M.size() - 1;
      while (l <= r) {
        ll mid = l + (r - l) / 2;
        bool ok = M[mid].first >= val;
        if (ok) {
          r = mid - 1;
        } else {
          l = mid + 1;
