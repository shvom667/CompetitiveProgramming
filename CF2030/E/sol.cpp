...
auto solve2(ll n, Vec<ll> a) {
    vector<Vec<Mint>> dp(n, vector<Mint> (n + 1, 0));
    Vec<ll> c(n, 0);
    for (auto&x : a) {
        c[x]++;
    }
    ...
    for (ll i = 1; i < n; i++) {
        for (ll j = n; j >= 0; j--) {
            for (ll k = j + 1; k <= n; k++) {
                ...
                dp[i][j] += table.choose(c[i], j) * dp[i - 1][k];
            }
            for (ll k = j; k <= n; k++) {
                dp[i][j] += dp[i -1 ][j] * table.choose(c[i], k);
            }
        }
    }
    Mint fans = 0;
...
auto solve(ll n, Vec<ll> a)
{
    ...
    vector<Mint> dp(c[0] + 1, 0);



    for (ll j = c[0]; j >= 0; j--) {
        dp[j] = table.choose(c[0], j);
        if (j + 1 <= c[0]) {
            dp[j] += dp[j + 1];
        }
        fans += (dp[j] - (j + 1 < dp.size() ? dp[j + 1] : Mint(0)) ) * (to.pow(sc[0 + 1])) * j;
    }
    ...
    for (ll i = 1; i < n; i++) {
        if (c[i] == 0) break;
        ll m = min(c[i] + 1, n / i + 1);
        dbg(m);
        vector<Mint> ndp(m, 0);
        vector<Mint> choose_sum(c[i] + 1, 0);

        for (ll j = c[i]; j >= 0; j--) {
            choose_sum[j] += table.choose(c[i], j);
            if (j + 1 <= c[i]) {
                choose_sum[j] += choose_sum[j + 1];
            }
        }
        for (ll j = ndp.size()-1; j >= 0; j--) {
            ndp[j] = table.choose(c[i], j) * (j + 1 < dp.size() ? dp[j + 1]: Mint(0)) + 
                    (j >= (ll)dp.size() ? Mint(0) : (dp[j] - (j + 1 < dp.size() ? dp[j + 1]: Mint(0)))) * choose_sum[j];
            if (j + 1 < ndp.size()) {
                ndp[j] += ndp[j + 1];
            }
        }
        dp = ndp;
        for (ll j = (ll) dp.size() - 1; j >= 0; j--) {
            fans += (dp[j] - (j + 1 < dp.size() ? dp[j + 1] : Mint(0)) ) *(to.pow(sc[i + 1])) * j;
        }
    }
