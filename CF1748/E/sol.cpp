...
auto solve(ll n, ll m, vector<ll> a) {
    ...
    vector<ll> L(n, 0), R(n, 0), nge(n, n), pge(n, -1);
    ...
    vector<ll> si(n, -1), pi(n, -1);
    for (ll i = 0; i < n; i++) {
        if (i == 0 || endIdx[i-1].empty()) {
            si[i] = -1;
        } else {
            si[i] = endIdx[i-1][0];
        }
    ...
    for (ll i = 0; i < n; i++) {
        pi[i] = -1;
        auto it = upper_bound(begin(endIdx[R[i]]),end(endIdx[R[i]]),i);
        if (it == end(endIdx[R[i]])) {
    ...
    Vec<Vec<Mint>> just(n, Vec<Mint> (m + 1, 0)), upto(n, Vec<Mint> (m + 1, 0));
    ...
    for (auto& i : I) {
        for (ll k = 1; k <= m; k++) {
            if (si[i] == -1) {
                ...
                just[i][k] = upto[si[i]][k-1];
            }
            upto[i][k] = just[i][k] * (pi[i] == -1 ?
                one : upto[pi[i]][k]
            );
            upto[i][k] += upto[i][k-1];
        }
    ...
    return upto[endIdx[n-1].front()][m].get_val();
}
