...
void solve() {
	...
	N *= 2;
	Vec<i64> A(N), oth(N);
	Vec<pair<i64, i64>> intervals;
	i64 s = 0, e = -1;
	map<i64, i64> mp;
	Vec<Vec<i64>> pos (N / 2 + 1, Vec<i64> (2, -1));
	REP (i, N) {
		cin >> A[i];
		if (pos[A[i]][0] == -1) {
			pos[A[i]][0] = i;
	...
	REP (i, N) {
		oth[i] = (pos[A[i]][0] == i ? pos[A[i]][1] : pos[A[i]][0]);
	}
	dbg(intervals);
	dbg(A);
	dbg(pos);
	...
	Lsegtree<pair<i64, i64>, i64> S(N, make_pair(INF64, -INF64), -1ll);
	// Vec<pair<i64,i64>> dummy;
	...
	for (auto& [s, e] : intervals) {
		...
		for (i32 i = s; i <= e; i++) {
			...
			while (true) {
				pair<i64, i64> cur = S.query(l, r);
				i64 nl = cur.first;
				i64 nr = cur.second;
				if (nr - nl + 1 == r - l + 1 && nr - nl + 1 != e - s + 1) {
					break;
				}
				if (nr - nl + 1 == e - s + 1) {
					cnt++;
